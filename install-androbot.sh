...
#   Auto-fix: jimp version conflict, lock file conflict,
#             sqlite3 build error & other dependency issues
# ============================================================
...
# Hapus node_modules & lock file lama
# PENTING: lock file (yarn.lock / package-lock.json) menyimpan versi exact
# dari saat developer pertama build — termasuk jimp 0.16.0 yang bermasalah.
# Jika tidak dihapus, patch package.json kita akan DIABAIKAN.
if [ -d "node_modules" ]; then
    warn "node_modules lama ditemukan, menghapus..."
    rm -rf node_modules
    log "node_modules dihapus."
fi

if [ -f "yarn.lock" ]; then
    warn "yarn.lock ditemukan — menghapus agar patch package.json efektif..."
    rm -f yarn.lock
    log "yarn.lock dihapus."
fi

if [ -f "package-lock.json" ]; then
    warn "package-lock.json ditemukan — menghapus agar patch package.json efektif..."
    rm -f package-lock.json
    log "package-lock.json dihapus."
fi
...
# --no-lockfile  : jangan buat yarn.lock baru (pakai versi dari package.json saja)
# --ignore-scripts: skip postinstall scripts yang sering gagal di Termux
info "Mencoba install dengan yarn (tanpa lock file)..."

if yarn install --no-lockfile --ignore-scripts 2>/dev/null; then
    log "Dependensi berhasil diinstall dengan yarn."

    # Build sqlite3 saja (native module yang paling sering butuh rebuild)
    ...
    # --no-package-lock : ekuivalen --no-lockfile di npm
    if npm install --no-package-lock --ignore-scripts 2>/dev/null; then
        ...
        # Pisahkan sqlite3 dulu karena paling sering jadi penyebab gagal total
        node -e "
const fs = require('fs');
const pkg = JSON.parse(fs.readFileSync('package.json', 'utf8'));
...
console.log('sqlite3 dipisahkan sementara dari package.json');
"
        yarn install --no-lockfile --ignore-scripts 2>/dev/null || \
        npm install --no-package-lock --ignore-scripts 2>/dev/null

        # Restore sqlite3 dan install terpisah
        SQLITE_VER=$(cat .sqlite3_version 2>/dev/null || echo "^5.1.6")
        info "Mencoba install sqlite3 $SQLITE_VER secara terpisah..."
        yarn add "sqlite3@$SQLITE_VER" --no-lockfile --ignore-scripts 2>/dev/null || \
        npm install "sqlite3@$SQLITE_VER" --no-package-lock --ignore-scripts 2>/dev/null || \
...
if node -e "const jimp = require('jimp'); console.log('jimp OK, versi:', jimp.version || 'unknown');" 2>/dev/null; then
    ...
    for JIMP_VER in "0.22.12" "0.21.3" "0.16.13" "0.16.2"; do
        ...
        yarn add "jimp@$JIMP_VER" --no-lockfile --ignore-scripts 2>/dev/null || \
        npm install "jimp@$JIMP_VER" --no-package-lock --ignore-scripts 2>/dev/null
